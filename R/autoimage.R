...
#' @param col A list of colors such as those generated by \code{\link[grDevices]{rainbow}}, \code{\link[grDevices]{heat.colors}}, \code{\link[grDevices]{topo.colors}}, \code{\link[grDevices]{terrain.colors}} or similar functions.  The default is given by \code{heat.colors(12)}.
...
#' @param map.poly A list with named elements \code{x} and \code{y} that are used to plot relevant polygons on each image using the \code{\link[graphics]{lines}} function.
...
#' @param map.points A list with named elements \code{x} and \code{y} that are used to plot relevant points on each image using the \code{\link[graphics]{points}} function.
...
#' @references Portions of the code for this function is inspired by the internals of the \code{\link[fields]{image.plot}} function written by Doug Nychka and from the \code{image.scale.2} function written by Marc Taylor and discussed at \code{http://menugget.blogspot.com/2013/12/new-version-of-imagescale-function.html}.
...
#' @export
autoimage = function(x, y, z, col = heat.colors(12), legend = TRUE, horizontal = TRUE, common.legend = TRUE, size = c(1, 1), mratio = 5, project = FALSE, map.grid = FALSE, mmar, legend.mar, axis.args, project.args, grid.args, map.poly, poly.args, map.points, points.args, outer.title, oma, mtext.args, ...){
  # obtain elements of ...
  args = list(...)
